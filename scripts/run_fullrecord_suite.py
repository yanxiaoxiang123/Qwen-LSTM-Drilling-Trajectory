...
def build_suite() -> list[dict[str, object]]:
    ...
    for short, target in targets.items():
        for model in numeric_models:
            suite.append(
                {
                    "name": f"online_16A_{short}_{model}_h5_fullrecord",
                    "target": target,
                    "model": model,
                }
            )
        for mode, model_dir in [
            ...
            item = {
                "name": f"online_16A_{short}_qwen_lstm_{mode}_h5_fullrecord",
                "target": target,
                "model": "qwen_lstm",
                "text_mode": mode,
            }
        ...
        suite.append(
            {
                "name": f"online_16A_{short}_qwen_itransformer_finetuned_h5_fullrecord",
                "target": target,
        ...
        suite.append(
            {
                "name": f"static_16A_{short}_qwen_lstm_finetuned_h5_fullrecord",
                "target": target,
                "model": "qwen_lstm",
                "text_mode": "finetuned",
                "qwen_model_dir": FINETUNED_QWEN,
...
def command_for(exp: dict[str, object], runs_dir: str) -> list[str]:
    ...
    cmd = [
        ...
        "30",
        "--online-epochs",
        str(exp.get("online_epochs", "1")),
        "--online-window-labels",
        "300",
        "--max-update-windows",
        "256",
        "--layers",
        ...
        "0.1",
        "--lr",
        "3e-4" if model in {"transformer", "itransformer", "qwen_itransformer"} else "1e-3",
        "--progress-interval",
        "500",
        "--rolling-windows",
        "100,300,500",
...
def main() -> None:
    ...
    parser.add_argument("--runs-dir", default="runs_online_16A_fullrecord_paper")
